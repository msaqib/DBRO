...
#csv_data = csv.reader(file('/home/faban/Python-Postgresql/Crime.csv'))
 ...
 session.execute("Drop table if exists Crime")
#print("Table created successfully")
 session.execute("create table Crime(a text primary key,b text, c text,d text,e text, f text,g text,h text, i text)")
 print('Table Created Successfully')
 count=0
 for row in csv_data:
 #print(row)
  ...
  count=1+count
  strCQL = "INSERT INTO Crime (a,b,c,d,e,f,g,h,i) VALUES (?,?,?,?,?,?,?,?,?)"
  pStatement = session.prepare(strCQL)
  session.execute(pStatement,[a,b,c,d,e,f,g,h,i])
